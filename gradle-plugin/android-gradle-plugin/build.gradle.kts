...
group = "cn.ktorfitx.android.gradle.plugin"
version = ktorfitxVersion

gradlePlugin {
	plugins {
		create("KtorfitxAndroidGradlePlugin", Action<PluginDeclaration> {
			id = "cn.ktorfitx.android"
			displayName = "Ktorfitx Android Gradle Plugin"
			implementationClass = "cn.ktorfitx.android.gradle.plugin.KtorfitxAndroidPlugin"
		})
	}
}

dependencies {
	implementation(libs.bundles.android.gradle.plugin)
}
...
mavenPublishing {
	...
	coordinates("cn.ktorfitx", "android-gradle-plugin", ktorfitxVersion)
	
	pom {
		name.set("android-gradle-plugin")
