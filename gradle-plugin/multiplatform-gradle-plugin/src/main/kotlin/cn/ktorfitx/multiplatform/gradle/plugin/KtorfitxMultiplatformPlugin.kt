...
class KtorfitxMultiplatformPlugin : Plugin<Project> {
    ...
    override fun apply(target: Project) = with(target) {
        ...
        afterEvaluate {
            ...
            pluginManager.checkPlugin("org.jetbrains.kotlin.multiplatform")
    ...
    private fun KotlinDependencyHandler.implementation(
        path: String,
        isDevelopmentMode: Property<Boolean>
    ): Dependency? {
        return if (isDevelopmentMode.get()) {
            this.implementation(project(":$path"))
        } else {
            this.implementation("${KtorfitxConstrants.GROUP_NAME}:$path:${KtorfitxVersions.KTORFITX}")
        }
    }
}